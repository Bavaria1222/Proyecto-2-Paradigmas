...
const Prioridad = () => {
  ...
  const handleSave = async () => {
    ...
    if (userId) {
      try {
        console.log('Updating prioridad:', dataToSend);
        ...
        console.log('Error updating prioridad:', error);
      }
    } else {
      try {
        console.log('Creating prioridad:', dataToSend);
        // Aquí va la llamada a la API para crear, por ejemplo:
        await PostPrioridad(dataToSend);
      } catch (error) {
        console.log('Error creating prioridad:', error);
      }
    }
  };

  const navigateAfterSuccess = () => {
    navigate(routes.prioridads);
  };
  ...
  const breadcrumbs = [
    { label: 'Prioridades', bold: true, route: routes.prioridads, underline: true },
    { label: `${titulo} prioridad`, bold: true },
  ];
  return (
    <CandyPage
      title={`${titulo} Prioridad`}
